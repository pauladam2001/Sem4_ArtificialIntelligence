...
def visualize(currentMap,x,y):
    screen = initPyGame((currentMap.n * 20, currentMap.m * 20))

    drona = pygame.image.load("E:\\AAFacultate\\Anul 2 Semestrul 2\\Artificial Intelligence\\Labs\\Lab 3 - Drone Exploration - EA - Copy\\drona.png")

    screen.blit(image(currentMap), (0, 0))
    startBrick = pygame.Surface((20, 20))
    startBrick.fill(BLACK)
    interBrick = pygame.Surface((20, 20))
    interBrick.fill(BLUE)
    if currentMap.surface[x][y] == 1:
        screen.blit(interBrick, (x*20, y*20))
    else:
