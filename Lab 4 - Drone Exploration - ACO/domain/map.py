...
class Map:
    ...
    def Sensors(self, list_of_sensors):
        ...
        for sensor in list_of_sensors:
            self.surface[sensor[0]][sensor[1]] = 2          # sensors are represented with '2' on the surface
    ...
    def image_path(self, visited, finalX=-1,  finalY=-1):
        ...
        seen_by_sensor.fill(DARKYELLOW)
        ...
        for i in range(self.n):
            for j in range(self.m):
                ...
                elif self.surface[i][j] == 3:   # squares seen by sensors
                    imagine.blit(seen_by_sensor, (j * 20, i * 20))
                elif self.surface[i][j] == 2:   # sensor
                    imagine.blit(sensor, (j * 20, i * 20))
                if self.surface[i][j] == 1:     # wall
